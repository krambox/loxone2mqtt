...
var WebSocketAPI = function (config, log) {
  ...
  var client = new NodeLoxWsApi(config.loxone, config.userid, config.password, true, wsAuth);
  ...
  client.on('send', function (message) {
    if (verbose) log.debug('WebSocketAPI - send message: ' + message);
  });
  ...
  client.on('message_text', function (message) {
    ...
    if (verbose) log.debug('WebSocketAPI - received text message: ', data);
  });
  ...
  client.on('message_file', function (message) {
    if (verbose) {
      ...
      log.debug('WebSocketAPI - received file: ', data.length);
    }
  });
  ...
  client.on('keepalive', function (time) {
    if (verbose) log.debug('WebSocketAPI - keepalive (' + time + 'ms)');
  });

  client.on('message_header', function (header) {
    if (verbose) log.debug('WebSocketAPI - received message header (' + header.next_state() + '):', header);
  });

  client.on('message_event_table_values', function (messages) {
    if (verbose) log.debug('WebSocketAPI - received value messages:', messages.length);
  });

  client.on('message_event_table_text', function (messages) {
    if (verbose) log.debug('WebSocketAPI - received text messages:', messages.length);
  });

  client.on('get_structure_file', function (data) {
    if (verbose) log.debug('WebSocketAPI - get structure file ' + data.lastModified);
  });
