...
var WebSocketAPI = function (config, app) {
  ...
  client.on('message_text', function (message) {
    ...
    switch (message.type) {
      case 'json':
        data.json = limitString(JSON.stringify(message.json), textLoggerLimit);
        break;
      case 'control':
        ...
        break;
      default:
        data.text = limitString(message.data, textLoggerLimit);
    }
  ...
  client.on('message_file', function (message) {
    ...
    switch (message.type) {
      case 'json':
        data.json = limitString(JSON.stringify(message.data), textLoggerLimit);
        break;
      case 'binary':
        data.length = limitString(message.data, textLoggerLimit);
        break;
      default:
        data.text = limitString(message.data, textLoggerLimit);
    }
  ...
  function updateEevent (uuid, evt) {
    var data = {
      uuid: uuid,
      'event': limitString(JSON.stringify(evt), textLoggerLimit)
    };
